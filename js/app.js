...
function inicializarSistema() {
    if (!localStorage.getItem('mp_data')) {
        const emptyData = {
            ...
            chamados: [], ordensServico: [], condominios: [], equipamentos: []
        };
...
function renderizarTabelaChamados() {
    ...
    if (dados.chamados.length === 0) {
        tbody.innerHTML = `<tr><td colspan="6" style="text-align:center; color: var(--text-muted);">Nenhum chamado registrado.</td></tr>`; return;
    }
...
function renderizarTabelaOS() {
    ...
    [...dados.ordensServico].reverse().forEach(os => {
        ...
        tr.innerHTML = `<td>#${os.id}</td><td><strong>${os.condominio}</strong></td><td>${os.servico}</td><td style="font-size: 13px;">${exibeDataHora}</td>
            <td>${os.tecnico}</td><td><span class="badge ${badgeStatus}">${os.status}</span></td>
            <td style="text-align: right;"><button class="icon-btn"><span class="material-symbols-outlined">visibility</span></button></td>`;
        tbody.appendChild(tr);
    });
}
...
function renderizarAgendaRotas() {
    ...
    if(ordens.length === 0) {
        container.innerHTML = `<div style="text-align: center; padding: 40px 20px;"><p style="color: var(--text-muted);">Nenhuma rota agendada.</p></div>`; return;
    }
    ...
    ordens.forEach(os => {
        ...
        item.innerHTML = `<div class="timeline-dot">${passo}</div>
            <div class="timeline-content"><div class="timeline-header"><span class="timeline-time">${os.hora}</span>
            <span style="color: ${colorStatus}; font-weight: 600;">${os.status}</span></div>
            <div class="timeline-title">${os.condominio}</div><div class="timeline-desc">${os.servico} (OS #${os.id})</div></div>`;
...
function configurarModuloTecnico() {
    ...
    const fecharBtn = document.getElementById('btn-fechar-modal-exec');
    if(fecharBtn) fecharBtn.addEventListener('click', () => document.getElementById('modal-executar-os').classList.add('hidden'));
    const btnIn = document.getElementById('btn-iniciar-servico');
    if(btnIn) btnIn.addEventListener('click', () => { atualizarStatusOSTecnico('Em andamento'); });
    const btnFi = document.getElementById('btn-finalizar-servico');
    if(btnFi) btnFi.addEventListener('click', () => {
...
function renderizarAgendaTecnico() {
    ...
    [...ordens].reverse().forEach(os => {
        ...
        card.innerHTML = `<div class="task-header"><span class="task-id">OS #${os.id}</span><span class="badge ${bg}">${os.status}</span></div>
...
function renderizarTabelaCondominios() {
    ...
    if (dados.condominios.length === 0) {
        tbody.innerHTML = `<tr><td colspan="6" style="text-align:center; color: var(--text-muted);">Nenhum condomínio cadastrado.</td></tr>`; return;
    }
    [...dados.condominios].reverse().forEach(cond => {
        const tr = document.createElement('tr');
        let badgeStatus = cond.status === 'Ativo' ? 'badge-status-ativo' : 'badge-status-inativo';
        tr.innerHTML = `<td><strong>${cond.nome}</strong></td><td>${cond.sindico}</td>
            <td style="font-size: 13px; color: var(--text-muted);">${cond.telefone}</td><td style="font-size: 13px;">${cond.endereco}</td>
...
function configurarModuloEquipamentos() {
    ...
    const modalEquip = document.getElementById('modal-novo-equipamento');
    const formEquip = document.getElementById('form-novo-equipamento');
    ...
    formEquip.addEventListener('submit', (e) => {
        ...
        dados.equipamentos.push({
            ...
            condominio: condNome,
            local: document.getElementById('input-equip-local').value,
...
function renderizarTabelaEquipamentos() {
    ...
    [...dados.equipamentos].reverse().forEach(eq => {
        ...
        let badgeStatus = 'badge-equip-inativo';
        if (eq.status === 'Funcionando') badgeStatus = 'badge-equip-func';
        ...
        tr.innerHTML = `
            ...
            <td><span class="badge ${badgeStatus}">${eq.status}</span></td>
            <td style="text-align: right;"><button class="icon-btn"><span class="material-symbols-outlined">visibility</span></button></td>
        `;
        tbody.appendChild(tr);
    });
